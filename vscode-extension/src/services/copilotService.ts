...
import { NorminetteService, NormError, NORM_RULES } from './norminetteService';

// ============================================================================
// COPILOT INTEGRATION SERVICE - Integracao com GitHub Copilot e Chat API
// ============================================================================
...
const PISCINE_EXERCISES: Record<string, { prototype: string; description: string; hints: string[] }> = {
    'ft_putchar': {
...
export class CopilotService {
    ...
    private chatParticipantDisposable?: vscode.Disposable;
    ...
    async getCopilotAvailable(): Promise<boolean> {
    ...
    private async handleHintCommand(
        stream: vscode.ChatResponseStream,
        prompt: string
    ): Promise<vscode.ChatResult> {
    ...
    private async handleRulesCommand(
        stream: vscode.ChatResponseStream,
        prompt: string
    ): Promise<vscode.ChatResult> {
        ...
        const relevantRules = Object.entries(NORM_RULES).filter(([code, info]) => {
    ...
    private applyBasicFixes(code: string): string {
        let fixed = code;
    ...
    private explainFixes(original: string, fixed: string): string {
    ...
    private explainLine(line: string): string | null {
    ...
    private findPotentialBugs(code: string): string[] {
        const issues: string[] = [];
    ...
    private extractExerciseName(prompt: string): string | null {
    ...
    private get42Snippets(): Array<{ label: string; body: string; description: string }> {
        return [
            {
                label: 'ft_header',
                body: `/* ************************************************************************** */
